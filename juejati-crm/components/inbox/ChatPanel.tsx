...
export function ChatPanel({ contactId, contactName, contactPhone }: Props) {
  ...
  if (!contactId) {
    return (
      <div className="flex-1 flex items-center justify-center text-fg-subtle bg-bg">
        <div className="text-center">
          <p className="text-lg font-medium text-fg">Seleccioná un lead</p>
  ...
  return (
    <div className="flex-1 flex flex-col bg-bg h-full overflow-hidden">
      <div className="px-4 py-3 bg-surface border-b border-border flex items-center justify-between shrink-0">
        <div>
          <p className="font-medium text-fg">{contactName}</p>
          {contactPhone && (
            <a
              href={`https://wa.me/${contactPhone.replace(/\D/g, '')}`}
              target="_blank"
              rel="noreferrer"
              className="text-2xs font-mono tabular-nums text-fg-muted hover:text-brand"
            >
              {contactPhone}
            </a>
          )}
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
        {loading && messages.length === 0 && (
          <p className="text-center text-sm text-fg-subtle">Cargando mensajes...</p>
        )}
        {error && <p className="text-center text-sm text-danger">{error}</p>}
        {!loading && !error && messages.length === 0 && (
          <p className="text-center text-sm text-fg-subtle">Sin mensajes</p>
        )}
        {messages.map((msg) => {
          ...
          return (
            <div key={msg.id} className={`flex flex-col ${isOut ? 'items-end' : 'items-start'}`}>
              <div
                className={`max-w-[75%] px-3 py-1.5 text-sm leading-snug whitespace-pre-wrap break-words ${
                  isOut
                    ? 'bg-brand text-brand-fg rounded-2xl rounded-br-md'
                    : 'bg-surface border border-border text-fg rounded-2xl rounded-bl-md'
                }`}
              >
                {msg.body}
              </div>
              <div className="flex items-center gap-2 mt-1 px-1">
                ...
                <span className="text-2xs font-mono text-fg-subtle tabular-nums">{formatTime(msg.createdAt)}</span>
              </div>
            </div>
          )
        })}
      ...
      <form onSubmit={handleSend} className="border-t border-border bg-surface p-3 shrink-0">
        ...
        <p className="text-2xs text-fg-subtle mt-1.5">Enter para enviar · Shift+Enter para salto de línea</p>
      </form>
    </div>
  )
}
